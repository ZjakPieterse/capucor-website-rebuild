-- ─── Migration 002: Explicit bracket prices ───────────────────────────────────
-- Replaces the base_price + scale_per_bracket formula with exact prices stored
-- per bracket per tier, matching the canonical price list PDF exactly.

-- 1. Add explicit price columns (nullable first so we can populate them)
alter table public.brackets
  add column basic_price    numeric(10,2),
  add column pro_price      numeric(10,2),
  add column premium_price  numeric(10,2);

-- 2. Wipe old brackets — labels, ordinals, and counts are all changing
delete from public.brackets;

-- 3. Accounting brackets (ordinals 0–25: Dormant + 25 revenue ranges)
...
-- 4. Bookkeeping brackets (ordinals 0–59: Dormant + Up to 50…1500 in steps of 25)
insert into public.brackets
  (service_slug, ordinal, label, is_enterprise, display_order, basic_price, pro_price, premium_price)
values
...
-- 5. Payroll brackets (ordinals 0–100: Dormant + Employees 1–100)
insert into public.brackets
  (service_slug, ordinal, label, is_enterprise, display_order, basic_price, pro_price, premium_price)
values
...
-- 6. Make price columns NOT NULL now that all rows are populated
alter table public.brackets
  alter column basic_price    set not null,
  alter column pro_price      set not null,
  alter column premium_price  set not null;

-- 7. Drop formula columns from services — no longer used for price calculation
alter table public.services
  drop column base_price,
  drop column scale_per_bracket;
